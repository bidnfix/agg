...
public interface PricingDAO extends CrudRepository<Pricing, Long>{
	...
	List<PricingDO> findCoverageLevelPricing(@Param("condition") byte condition, @Param("groupId") int groupId, @Param("deductibleAmt") int deductibleAmt, 
			@Param("coverageTerm") int coverageTerm);
	...
	@Query("SELECT new com.agg.application.model.PricingDO(p.id.cLevelHours, p.ptBasePrice, p.phBasePrice, p.plBasePrice) "
			...
			+ "and p.id.coverageTerm = :coverageTerm "
			+ "and p.id.deductibleAmount = :deductibleAmt "
			+ "and p.id.cLevelHours = :coverageHours")
	List<PricingDO> findCoverageLevelPricing(@Param("condition") byte condition, @Param("groupId") int groupId, @Param("deductibleAmt") int deductibleAmt, 
			@Param("coverageTerm") int coverageTerm, @Param("coverageHours") int coverageHours); 
